...
Write a and call a function called "rgb_hex" that takes 
a parameter called "color." The function should be able 
to intake a color in rgb format and return it in hex 
format as well as vice versa. A sample of color conversions 
are below. Write in logic so the function can interpret 
rgb and hex colors entered in different formats such as 
"rgb_hex((255,255,255))" or "rgb_hex('ffffff')". Also make 
sure to account for someone inputting a color not in the 
database.
"""
...
def rgb_hex(color):
	...
	for k, v in color_conversions.items():
		if color == k:
			...
			err = '''Sorry we don\'t have that color on file. 
			Please make sure you entered the color with its 
			correct syntax.'''
			return err
